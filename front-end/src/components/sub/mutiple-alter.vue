    ...
    <Form :model="question" :label-width="80">
        ...
        <Form-item label="正确答案">
            <Checkbox-group v-model="question.answer">
                <Checkbox :label="index" v-for="(item, index) in question.selections" :key="index">{{ index | indexToSelection('选项') }}</Checkbox>
            </Checkbox-group>
        </Form-item>
        <Form-item>
...
export default {
  ...
  methods: {
    ...
    submit () {
      ...
      new MutipleIO().addMutiple(this.question).then(res => {
        alert('success')
      }).catch(err => {
        alert('err')
      })
